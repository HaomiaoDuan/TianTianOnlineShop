...
import com.tiantian.sso.component.JedisClient;
...
public class LoginServiceImpl implements LoginService {
    ...
    public TiantianResult login(String username, String password, HttpServletRequest request,
        ...
        if (list == null || list.isEmpty()) {
            return TiantianResult.build(400, "用户名或密码错误");
        }
        ...
        // value:user转json
        user.setPassword(null); // 清空密码更安全
        jedisClient.set(REDIS_SESSION_KEY + ":" + token, JsonUtils.objectToJson(user));
        // 设置session的过期时间
        jedisClient.expire(REDIS_SESSION_KEY + ":" + token, SESSION_EXPIRE);
        // 写cookie
        CookieUtils.setCookie(request, response, "TT_TOKEN", token);// 关闭浏览器token的cookie就过期了

        return TiantianResult.ok(token);
    }
    ...
    public TiantianResult getUserByToken(String token) {
        ...
        if (StringUtils.isBlank(json)) {
            return TiantianResult.build(400, "用户session已经过期");
        }
        // 把json转换成java对象
